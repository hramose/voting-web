    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <ul class="nav nav-tabs">
             ...
             <li role="presentation" class="active"><a href="{{route('candidate_list')}}">Candidates List</a></li>
            </ul>
           ...
           <div class="col-md-3">
                 <form action="{{route('candidate_list_search')}}" method="POST">
            ...
            <table class="table">
                ...
                <tbody>
                    @foreach($candidate as $can)
                        <tr>
                            <td>{{$can->user->class_id}}</td>
                            <td>{{$can->party->name}}</td>
                            <td>{{$can->position->position}}</td>
                            <td>{{$can->user->fname}} {{$can->user->lname}}</td>
                            <td>{{$can->user->grade}}</td>
                            <td>{{$can->user->section}}</td>
                            <td>{{$can->user->email}}</td>
                            <td>
                               <form action="{{route('candidate_delete', ['id'=> $can->user_id])}}" method="get" id="formDel{{$can->user_id}}">

                                <button type="button" value="{{$can->user_id}}" class="btn btn-danger btn-xs delMe">
...
<script type="text/javascript">
    $(document).ready(function(){
        $(".delMe").click(function(){
            ...
            swal({
              title: "Are you sure?",
              text: "Once deleted, candidate can no longer access his/her account!",
              icon: "warning",
              buttons: true,
              dangerMode: true,
            })
